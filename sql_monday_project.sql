...
WITH city_table AS (
    SELECT 
        ...
        COUNT(DISTINCT s.customer_id) AS total_cx,
        ROUND(
            CAST(SUM(s.total) AS FLOAT) / NULLIF(CAST(COUNT(DISTINCT s.customer_id) AS FLOAT), 0), 
            2
        ) AS avg_sale_pr_cx
    FROM sales AS s
    JOIN customers AS c ON s.customer_id = c.customer_id
...
SELECT 
    ...
    ct.avg_sale_pr_cx,
    ROUND(
        CAST(cr.estimated_rent AS FLOAT) / NULLIF(CAST(ct.total_cx AS FLOAT), 0), 
        2
    ) AS avg_rent_per_cx
...
SELECT
    ...
    last_month_sale,
    ROUND(
        CAST(cr_month_sale - last_month_sale AS FLOAT) / NULLIF(CAST(last_month_sale AS FLOAT), 0) * 100, 
        2
    ) AS growth_ratio
FROM growth_ratio
